...
router.post('/:id/export-pdf', (req, res) => {
  ...
  const anamnesis = row.anamnesis ? JSON.parse(row.anamnesis) : null;
  const hypotheses = row.hypotheses ? JSON.parse(row.hypotheses) : [];
  const exams = row.exams ? JSON.parse(row.exams) : [];
  let treatment = row.treatment;
  try { treatment = JSON.parse(treatment); } catch (e) { /* keep as string */ }
  ...
  if (anamnesis) {
    ...
    for (const [label, value] of anamnesisFields) {
  ...
  if (hypotheses.length > 0) {
    ...
    for (const h of hypotheses) {
      ...
      if (h.justification) {
        doc.font('Helvetica').text(`  ${h.justification}`);
      }
      doc.moveDown(0.3);
    }
    doc.moveDown(0.5);
  ...
  if (treatment) {
    ...
    if (typeof treatment === 'object') {
      ...
      doc.fontSize(11).font('Helvetica').text(treatment);
    }
    doc.moveDown(0.5);
  }
